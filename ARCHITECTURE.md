...
- New Game+ starts a clean run, then applies configured carryover from the selected completed save (`rules.json` `save.new_game_plus.carryover`).
...
- If multiple encounter zones overlap at the current position, the first matching zone in `map.encounters` order is used.
...
- Item usage supports `context: "none"` for non-usable items that should never expose a Use action.
- Consumable use is blocked when it would have no effect (full HP/MP heals, no matching status cure, no fallen ally for revive, duplicate spell/recipe learning).
...
- Combatants clear active statuses immediately when KO'd (HP reaches 0), including enemies.
